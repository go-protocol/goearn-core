...
contract StrategyCurveYBUSDVoterProxy {
    ...
    address public constant crv = address(0xD533a949740bb3306d119CC777fa900bA034cd52); //crv合约地址
    address public constant uni = address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); //uni兑换合约
    address public constant weth = address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); // used for crv <> weth <> dai route

    address public constant dai = address(0x6B175474E89094C44Da98b954EedeAC495271d0F); //dai合约地址
    address public constant ydai = address(0xC2cB1040220768554cf699b0d863A3cd4324ce32); //ydai合约地址
    address public constant curve = address(0x79a8C46DeA5aDa233ABaFFD40F3A0A2B1e5A4F27); //curve兑换合约

    address public constant gauge = address(0x69Fb7c45726cfE2baDeE8317005d3F94bE838840); //流动性挖CRV合约
    address public constant proxy = address(0x5886E475e163f78CF63d6683AbC7fe8516d12081); //策略合约地址
    address public constant voter = address(0xF147b8125d2ef93FB6965Db97D6746952a133934); //CurveYCRVVoter合约

    uint256 public keepCRV = 1000; //保留10%的CRV
    uint256 public constant keepCRVMax = 10000; //保留最多100%CRV

    uint256 public performanceFee = 500; //5%的绩效费
    uint256 public constant performanceMax = 10000; //最多100%绩效费

    uint256 public withdrawalFee = 50; //0.5%提现费用
    uint256 public constant withdrawalMax = 10000; //最大100%提现费用
    ...
    function withdraw(uint256 _amount) external {
        ...
        uint256 _balance = IERC20(want).balanceOf(address(this)); //存款币种在本合约中的余额
        if (_balance < _amount) {
            //余额小于提现数量
        ...
        uint256 _fee = _amount.mul(withdrawalFee).div(withdrawalMax); //计算取款费用
    ...
    function harvest() public {
        ...
        uint256 _crv = IERC20(crv).balanceOf(address(this)); //获得收获的CRV数量
        if (_crv > 0) {
            //判断收获的CRV数量是否大于0

            uint256 _keepCRV = _crv.mul(keepCRV).div(keepCRVMax); //计算需要保留的crv
        ...
        uint256 _dai = IERC20(dai).balanceOf(address(this)); //收获dai的余额
        if (_dai > 0) {
            //判断dai大于0
        ...
        uint256 _want = IERC20(want).balanceOf(address(this)); ////收获busd余额
        if (_want > 0) {
            uint256 _fee = _want.mul(performanceFee).div(performanceMax); //计算绩效费用
