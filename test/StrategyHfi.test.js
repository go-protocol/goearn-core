...
const { advanceBlockTo,latest,increase,duration } = require("./inc/time");
...
const BASE_TEN = 10;
const thisStrategy = "StrategyHfi";
...
describe("测试" + thisStrategy, function () {
    ...
    for (let i = 0; i < tokenJson[thisStrategy].length; i++) {
        ...
        it("lastBlock", async function () {
            lastBlock = await latest();
            console.log(
                "   lastBlock:",
                lastBlock.toString()
            );
        ...
        it("balanceOfPool 1", async function () {
        ...
        it("balanceOfPool 2", async function () {
            balanceOfPool = await this.strategies[want].balanceOfPool();
            console.log(
                "   balanceOfPool:",
                ethers.utils.formatEther(balanceOfPool).toString()
        ...
        it("poolEarned", async function () {
            poolEarned = await this.strategies[want].poolEarned();
            console.log(
                "   poolEarned:",
                ethers.utils.formatEther(poolEarned).toString()
            );
        });
