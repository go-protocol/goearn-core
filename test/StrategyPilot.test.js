...
const BASE_TEN = 10;
const thisStrategy = "StrategyPilot";
...
describe("测试" + thisStrategy, function () {
    ...
    for (let i = 0; i < tokenJson[thisStrategy].length; i++) {
        it("部署策略", async function () {
            pool = tokenJson[thisStrategy][i].pool;
            want = tokenJson[thisStrategy][i].want;
        ...
        it("存款", async function () {
            console.log('   depositAmount:',ethers.utils.formatEther(depositAmount).toString())
            if (want !== "WHT") {
        ...
        it("balanceOfWant", async function () {
            balanceOfWant = await this.strategies[want].balanceOfWant();
            expect(balanceOfWant).to.equal('0');
            console.log('   balanceOfWant:',ethers.utils.formatEther(balanceOfWant).toString());
        });

        it("balanceOfVault", async function () {
            balanceOfVault = await this.strategies[want].balanceOfVault();
            expect(balanceOfVault).to.equal('0');
            console.log('   balanceOfVault:',ethers.utils.formatEther(balanceOfVault).toString());
        });

        it("balanceOfPool", async function () {
            balanceOfPool = await this.strategies[want].balanceOfPool();
            expect(balanceOfPool).to.gt('0');
            console.log('   balanceOfPool:',ethers.utils.formatEther(balanceOfPool).toString());
        });
        ...
        it("vault.balance", async function () {
            expect(await this.vaults[want].balance()).to.gt(depositAmount);
        });
        
        it("balanceOf", async function () {
            balanceOf = await this.strategies[want].balanceOf();
            expect(balanceOf).to.gt('0');
            console.log('   balanceOf:',ethers.utils.formatEther(balanceOf).toString());
        });

        // it("取款withdrawFromPool", async function () {
        //     let tx = await this.strategies[want].withdrawFromPool();
        // });
        ...
        // it("取款之前withdrawFromVault", async function () {
        //     balanceOfVault = await this.strategies[want].balanceOfVault();
        //     expect(balanceOfVault).to.gt('0');
        //     console.log('   取款之前withdrawFromVault:',ethers.utils.formatEther(balanceOfVault).toString());
        // });

        // it("取款withdrawFromVault", async function () {
        //     let tx = await this.strategies[want].withdrawFromVault();
        // });

        // it("验证withdrawFromVault", async function () {
        //     balanceOfVault = await this.strategies[want].balanceOfVault();
        //     expect(balanceOfVault).to.equal('0');
        //     console.log('   验证withdrawFromVault:',ethers.utils.formatEther(balanceOfVault).toString());
        // });
